...
func (b *Buffer) ReadUint16(e byteorder.Endian) (v uint16, err error) {
	...
	case byteorder.LittleEndian:
		v = uint16(b.slice[b.readOffset]) | uint16(b.slice[b.readOffset+1])<<8
...
func (b *Buffer) ReadInt16(e byteorder.Endian) (v int16, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		v = int16(b.slice[b.readOffset]) | int16(b.slice[b.readOffset+1])<<8
	case byteorder.BigEndian:
...
func (b *Buffer) ReadUint24(e byteorder.Endian) (v uint32, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		v = uint32(b.slice[b.readOffset]) | uint32(b.slice[b.readOffset+1])<<8 | uint32(b.slice[b.readOffset+2])<<16
	case byteorder.BigEndian:
...
func (b *Buffer) WriteUint24(v uint32, e byteorder.Endian) (err error) {
	...
	case byteorder.LittleEndian:
		b.slice[b.writeOffset] = byte(v)
		b.slice[b.writeOffset+1] = byte(v >> 8)
		b.slice[b.writeOffset+2] = byte(v >> 16)
	case byteorder.BigEndian:
		b.slice[b.writeOffset+2] = byte(v)
		b.slice[b.writeOffset+1] = byte(v >> 8)
...
func (b *Buffer) ReadUint32(e byteorder.Endian) (v uint32, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		v = uint32(b.slice[b.readOffset]) | uint32(b.slice[b.readOffset+1])<<8 |
			uint32(b.slice[b.readOffset+2])<<16 | uint32(b.slice[b.readOffset+3])<<24
	case byteorder.BigEndian:
...
func (b *Buffer) WriteUint32(v uint32, e byteorder.Endian) (err error) {
	...
	case byteorder.LittleEndian:
		...
		b.slice[b.writeOffset+1] = byte(v >> 8)
		b.slice[b.writeOffset+2] = byte(v >> 16)
		b.slice[b.writeOffset+3] = byte(v >> 24)
	case byteorder.BigEndian:
		b.slice[b.writeOffset+3] = byte(v)
		b.slice[b.writeOffset+2] = byte(v >> 8)
...
func (b *Buffer) ReadInt32(e byteorder.Endian) (v int32, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		v = int32(b.slice[b.readOffset]) | int32(b.slice[b.readOffset+1])<<8 |
			int32(b.slice[b.readOffset+2])<<16 | int32(b.slice[b.readOffset+3])<<24
	case byteorder.BigEndian:
...
func (b *Buffer) ReadUint64(e byteorder.Endian) (v uint64, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		v = uint64(b.slice[b.readOffset]) | uint64(b.slice[b.readOffset+1])<<8 |
			uint64(b.slice[b.readOffset+2])<<16 | uint64(b.slice[b.readOffset+3])<<24 |
			uint64(b.slice[b.readOffset+4])<<32 | uint64(b.slice[b.readOffset+5])<<40 |
			uint64(b.slice[b.readOffset+6])<<48 | uint64(b.slice[b.readOffset+7])<<56
	case byteorder.BigEndian:
		v = uint64(b.slice[b.readOffset+7]) | uint64(b.slice[b.readOffset+6])<<8 |
			uint64(b.slice[b.readOffset+5])<<16 | uint64(b.slice[b.readOffset+4])<<24 |
...
func (b *Buffer) WriteUint64(v uint64, e byteorder.Endian) (err error) {
	...
	case byteorder.LittleEndian:
		...
		b.slice[b.writeOffset+2] = byte(v >> 16)
		b.slice[b.writeOffset+3] = byte(v >> 24)
		b.slice[b.writeOffset+4] = byte(v >> 32)
		b.slice[b.writeOffset+5] = byte(v >> 40)
		b.slice[b.writeOffset+6] = byte(v >> 48)
		b.slice[b.writeOffset+7] = byte(v >> 56)
	case byteorder.BigEndian:
		b.slice[b.writeOffset+7] = byte(v)
		b.slice[b.writeOffset+6] = byte(v >> 8)
		b.slice[b.writeOffset+5] = byte(v >> 16)
		b.slice[b.writeOffset+4] = byte(v >> 24)
		b.slice[b.writeOffset+3] = byte(v >> 32)
		b.slice[b.writeOffset+2] = byte(v >> 40)
...
func (b *Buffer) ReadInt64(e byteorder.Endian) (v int64, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		v = int64(b.slice[b.readOffset]) | int64(b.slice[b.readOffset+1])<<8 |
			int64(b.slice[b.readOffset+2])<<16 | int64(b.slice[b.readOffset+3])<<24 |
			int64(b.slice[b.readOffset+4])<<32 | int64(b.slice[b.readOffset+5])<<40 |
			int64(b.slice[b.readOffset+6])<<48 | int64(b.slice[b.readOffset+7])<<56
	case byteorder.BigEndian:
		v = int64(b.slice[b.readOffset+7]) | int64(b.slice[b.readOffset+6])<<8 |
			int64(b.slice[b.readOffset+5])<<16 | int64(b.slice[b.readOffset+4])<<24 |
...
func (b *Buffer) ReadFloat32(e byteorder.Endian) (v float32, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		bits := uint32(b.slice[b.readOffset]) | uint32(b.slice[b.readOffset+1])<<8 |
			uint32(b.slice[b.readOffset+2])<<16 | uint32(b.slice[b.readOffset+3])<<24
		v = math.Float32frombits(bits)
	case byteorder.BigEndian:
		bits := uint32(b.slice[b.readOffset+3]) | uint32(b.slice[b.readOffset+2])<<8 |
			uint32(b.slice[b.readOffset+1])<<16 | uint32(b.slice[b.readOffset])<<24
		v = math.Float32frombits(bits)
	default:
		return 0, errors.New(CPB_ERROR)
...
func (b *Buffer) WriteFloat32(v float32, e byteorder.Endian) (err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		b.slice[b.writeOffset] = byte(bits)
		b.slice[b.writeOffset+1] = byte(bits >> 8)
		b.slice[b.writeOffset+2] = byte(bits >> 16)
		b.slice[b.writeOffset+3] = byte(bits >> 24)
	case byteorder.BigEndian:
		b.slice[b.writeOffset+3] = byte(bits)
		b.slice[b.writeOffset+2] = byte(bits >> 8)
...
func (b *Buffer) ReadFloat64(e byteorder.Endian) (v float64, err error) {
	...
	switch e {
	case byteorder.LittleEndian:
		bits := uint64(b.slice[b.readOffset]) | uint64(b.slice[b.readOffset+1])<<8 |
			uint64(b.slice[b.readOffset+2])<<16 | uint64(b.slice[b.readOffset+3])<<24 |
			uint64(b.slice[b.readOffset+4])<<32 | uint64(b.slice[b.readOffset+5])<<40 |
			uint64(b.slice[b.readOffset+6])<<48 | uint64(b.slice[b.readOffset+7])<<56
		v = math.Float64frombits(bits)
	case byteorder.BigEndian:
		bits := uint64(b.slice[b.readOffset+7]) | uint64(b.slice[b.readOffset+6])<<8 |
			uint64(b.slice[b.readOffset+5])<<16 | uint64(b.slice[b.readOffset+4])<<24 |
			uint64(b.slice[b.readOffset+3])<<32 | uint64(b.slice[b.readOffset+2])<<40 |
			uint64(b.slice[b.readOffset+1])<<48 | uint64(b.slice[b.readOffset])<<56
		v = math.Float64frombits(bits)
...
func (b *Buffer) WriteFloat64(v float64, e byteorder.Endian) (err error) {
	...
	case byteorder.LittleEndian:
		...
		b.slice[b.writeOffset+2] = byte(bits >> 16)
		b.slice[b.writeOffset+3] = byte(bits >> 24)
		b.slice[b.writeOffset+4] = byte(bits >> 32)
		b.slice[b.writeOffset+5] = byte(bits >> 40)
		b.slice[b.writeOffset+6] = byte(bits >> 48)
		b.slice[b.writeOffset+7] = byte(bits >> 56)
	case byteorder.BigEndian:
		b.slice[b.writeOffset+7] = byte(bits)
		b.slice[b.writeOffset+6] = byte(bits >> 8)
		b.slice[b.writeOffset+5] = byte(bits >> 16)
		b.slice[b.writeOffset+4] = byte(bits >> 24)
		b.slice[b.writeOffset+3] = byte(bits >> 32)
		b.slice[b.writeOffset+2] = byte(bits >> 40)
